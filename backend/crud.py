...
# measurments # Create / ReadMany / readOne / Update / Delete 
def create_measurment(db: Session, measurment: schemas.MeasurmentCreate):
    db_measurment = models.Measurment(**measurment.dict())
    db.add(db_measurment)
    db.commit()
    db.refresh(db_measurment)
    return db_measurment

def get_measurments(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Measurment).offset(skip).limit(limit).all()

def get_measurment(db: Session, measurment_id: int):
    return db.query(models.Measurment).filter(models.Measurment.id == measurment_id).first()

def update_measurment(db: Session, measurment_id: int, measurment: schemas.MeasurmentCreate):
    db_measurment = get_measurment(db=db, measurment_id=measurment_id)
    measurment_data = measurment.dict(exclude_unset=True)
    for key, value in measurment_data.items():
        setattr(db_measurment, key, value)
    db.add(db_measurment)
    db.commit()
    db.refresh(db_measurment)
    return db_measurment

def delete_measurment(db: Session, measurment_id: int):
    db_measurment = get_measurment(db=db, measurment_id=measurment_id)
    db.delete(db_measurment)
    db.commit()
    return {"ok": True}
